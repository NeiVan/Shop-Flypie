<style>
#dropdown-language{
float: right;width: 88px;height: 20px;line-height: 20px;color: #aaa;display: inline-block;vertical-align:top;background-image:url(data:image/png;base64,);
background-position: 70px;background-repeat: no-repeat;background-size: 10px 6px;background-color: #262626;
}
</style>
<?php if (count($languages) > 1) { ?>
<!-- Language -->
<form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="language_form">
	<!--<div class="dropdown"> -->
	<span id="dropdown-language">
		<?php foreach ($languages as $language) { ?>
		<?php if ($language['code'] == $code) { ?>
		<a href="#" class="dropdown-toggle" data-hover="dropdown" data-toggle="dropdown"><?php echo $language['name']; ?></a>
		<?php } ?>
		<?php } ?>
		<ul class="dropdown-menu" style="top: -78px;">
		  <?php foreach ($languages as $language) { ?>
		  <li style="width: 100%"><a href="javascript:;" onclick="$('input[name=\'code\']').attr('value', '<?php echo $language['code']; ?>'); $('#language_form').submit();"><?php echo $language['name']; ?></a></li>
		  <?php } ?>
		</ul>
	</span>
	<!--</div> -->
	
	<input type="hidden" name="code" value="" />
	<input type="hidden" name="redirect" value="<?php echo $redirect; ?>" />
</form>
<?php } ?>