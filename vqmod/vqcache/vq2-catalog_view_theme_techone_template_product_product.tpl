...
<div itemscope itemtype="http://schema.org/Product">
  		...
  		<div class="col-sm-<?php if($theme_options->get( 'custom_block', 'product_page', $config->get( 'config_language_id' ), 'status' ) == 1 || count($product_custom_block)) { echo 9; } else { echo 12; } ?>">
  			<div class="row" id="quickview_product">
			    ...
			    if ($theme_options->get( 'product_image_size' ) == 1) {
			    	$image_grid = 6; $product_center_grid = 6;
			    }
			    ...
			    <div class="col-sm-<?php echo $product_center_grid; ?> product-center clearfix">
			     <div itemprop="offerDetails" itemscope itemtype="http://schema.org/Offer">
			      ...
			      <div class="description">
					...
					<span><?php if($lang=="cn"){echo '邮费: 免费';}else{echo 'Postage: Free';}?></span></div>
			      <?php if ($price) { ?>
			      <div class="price">
			        <?php if (!$special) { ?>
			        ...
			        <br />
			        <?php if ($tax) { ?>
			        <span class="price-tax"><?php echo $text_tax; ?> <span id="price-tax"><?php if($lang=="cn"){echo '已包括';}else{echo 'Included';}?></span></span><br />
			        <?php } ?>
			        <?php if ($points) { ?>
			      ...
			      <div class="cart">
			        <div class="add-to-cart clearfix">
			          ...
			          if( count($product_enquiry) ) {
						  ...
						  <input type="button" value="<?php echo $button_amazon; ?>" id="<?php if($lang=='cn'){ echo 'button-tianmao';}else{ echo 'button-amazon';} ?>"  data-loading-text="<?php echo $text_loading; ?>" class="button" />
     			          <?php
					...
					<div class="delivery-time"><span><?php if($lang=="cn"){echo '发货时间：订单付款后3-7个工作日。';}else{echo 'Delivery time: 3-7 business days after order payment.';}?></span></div>
			        <?php if ($minimum > 1) { ?>
  ...
  <?php
	  ...
	  if ($attribute_groups) { $tabs[] = array( 'heading' => $tab_attribute, 'content' => 'attribute','sort' => 2 ); }
$tabs[] = array( 'heading' => $tab_youtube, 'content' => 'video','sort' => 3 );
  ...
  <?php $i = 0; foreach($tabs as $tab) { $i++;
  	$id = 'tab_'.$i;
  	if($tab['content'] != 'description' && $tab['content'] != 'attribute' && $tab['content'] != 'review' && $tab['content'] != 'video') {
...
<div id="tab-video" class="tab-content" itemprop="video">
  ...
  <div class="box clearfix">
    ...
    <div class="box-heading"><?php echo $text_recommended_accessories; ?></div>
    <div class="strip-line"></div>
...
$('select[name=\'recurring_id\'], input[name="quantity"]').change(function(){
	$.ajax({
		url: 'product-getRecurringDescription.html',
		type: 'post',
		...
		success: function(json) {
			...
			if (json['success']) {
				$.notify({
					...
					onClosed: null,
					icon_type: 'class',
					template: '<div style="z-index:10000" data-notify="container" class="col-xs-11 col-sm-3 alert alert-success" role="alert">' +
				...
				$('#cart_block #cart_content').load('cart-info.html #cart_content_ajax');
				$('#cart_block #total_price_ajax').load('cart-info.html #total_price');
				$('#cart_block #cart_count_ajax').load('cart-info.html #cart_count');
...
$('#button-amazon').on('click', function() {
    window.open("https://www.amazon.com");
});
$('#button-tianmao').on('click', function() {
	window.open("https://feipai.tmall.com/");
});
//--></script>
<script type="text/javascript"><!--
$('.date').datetimepicker({
...
$('#review').load('product-review.html&product_id=<?php echo $product_id; ?>');

$('#button-review').on('click', function() {
    $.ajax({
        url: 'product-write.html&product_id=<?php echo $product_id; ?>',
        type: 'post',
        dataType: 'json',
