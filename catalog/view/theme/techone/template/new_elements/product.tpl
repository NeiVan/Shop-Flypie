	...
	<div class="left">
		<?php if ($product['thumb']) { ?>
			...
			<div class="image <?php if($theme_options->get( 'product_image_effect' ) == '1') { echo 'image-swap-effect'; } ?>">
				...
				<div class="quickview">
					<a href="product-quickview.html&product_id=<?php echo $product['product_id']; ?>" title="<?php echo $product['name']; ?>"><?php if($theme_options->get( 'quickview_text', $config->get( 'config_language_id' ) ) != '') { echo html_entity_decode($theme_options->get( 'quickview_text', $config->get( 'config_language_id' ) )); } else { if($config->get('config_language_id')=='2'){ echo '快速浏览';}else{echo 'Quickview';} } ?></a>
				</div>
				<?php } ?>
