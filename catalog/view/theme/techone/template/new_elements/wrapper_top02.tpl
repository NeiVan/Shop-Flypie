...
<div class="breadcrumb <?php if($theme_options->get( 'breadcrumb_layout' ) == 1) { echo 'full-width'; } else { echo 'fixed'; } ?>" style="padding: 0;">
	<div style="width: 100%;height: auto;margin-bottom: 10px;">
		<img alt="Flypie" src="https://www.flypie.net/image/catalog/Contact/flypie-contactus.png">
	</div>
</div>
			...
			<div class="container">
				<div class="clearfix">
					<ul>
						...
						<li><a href="<?php echo $breadcrumb['href']; ?>"><?php if($breadcrumb['text'] != '<i class="fa fa-home"></i>') { echo $breadcrumb['text']; } else { if($theme_options->get( 'home_text', $config->get( 'config_language_id' ) ) != '') { echo $theme_options->get( 'home_text', $config->get( 'config_language_id' ) ); } { echo $breadcrumb['text']; } } ?></a></li>
						<?php } ?>
					</ul>
				</div>
			</div>
